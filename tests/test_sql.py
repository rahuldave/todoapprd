...
from todoapp.sql import create_db, get_db, unget_db
...
def test_insert_todo_with_foreign_key(db):
    ...
    cursor.execute(
        "INSERT INTO todos (task, done, category_id) VALUES ('Clean the house', 0, ?)",
        (category_id,),
    )
    db.commit()
...
def test_query_todos(db):
    ...
    cursor.execute(
        "INSERT INTO todos (task, done, category_id) VALUES ('Clean the house', 0, 1)"
    )
    db.commit()
